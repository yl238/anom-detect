...
avg_precision_result = np.zeros([len(score_names), len(classes)+1])
...
for (name, cls) in product(score_names, classes):
    ...
    avg_precision_result[score_names.index(name), classes.index(cls)] = average_precision_score(y_true, y_score)
...
for name in score_names:
    ...
    avg_precision_result[score_names.index(name), -1] = average_precision_score(y_true, y_score)
...
df = pd.DataFrame(avg_precision_result, index=score_names, columns=classes + ['ALL'])
# display
print("###################### Precision #####################")
print(df)
print("####################################################")
df.to_csv('avg_precision.csv')
